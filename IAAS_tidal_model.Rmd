...
st_clean <- daily_stream %>% 
  mutate(mean = mean_va*0.028*1.157*10^5) %>% # convert mean flow from [ft^3 s^-1] to [m^3 d^-1]
...
```{r, message=FALSE}
# Data source: Frank, K. 2014-2015 He‘eia Fishpond Water Quality Report: Data, Visual Summary, and Brief Analysis; University of Hawai’i, Department of Oceanography, 2016
# Data used in parameterization and validation of the model
...
# Isolate temperature measurements [ºC] from the surface and bottom of the He‘eia Fishpond water column
```{r}
...
Fmsy_mugil = exp((exp(Predict_mugil[[1]]$Mean_pred["ln_Fmsy"]+0.5*diag(Predict_mugil[[1]]$Cov_pred)["ln_Fmsy"]))) %>% as.numeric()/365/4 # [(6h)^-1], maximum sustainable fishing rate
...
r_NChl = 0.2 # [g N (g Chla)^-1] ratio of nitrogen to Chl-a in algae
...
Ni_ocean = 0.0068 # [g m^-3] nitrate N
 ...
 # Nitrogen loading from current land use (taro agriculture and wetland)
...
pars_control = list(
  ...
  # Annual nitrogen concentration increases in current scenario
  No_inc = 0, # [g m^-3]
  Na_inc = 0, # [g m^-3]
  Ni_inc = 0 # [g m^-3]
)
...
pars_restored = list(
  ...
  # Annual nitrogen concentration increases in restored scenario
  No_inc = 0.22, # [g m^-3]
  Na_inc = 0.009, # [g m^-3]
  Ni_inc = 0.009 # [g m^-3]
)
...
pars_urban = list(
  ...
  # Annual nitrogen concentration increases in urban scenario
  No_inc = 0.56, # [g/m^3]
  Na_inc = 0.023, # [g/m^3]
  Ni_inc = 0.023 # [g/m^3]
)
...
```{r}
# Generate a forcing function for the daily average stream flow rates 
...
solveFish <- function(pars, t = times_run) {
  ...
  with(as.list(c(state, pars)), {
    ...
    # Placeholder nitrogen loading rates 
    W_No = (LU_No + No_inc*20)*Q_st # [g (6h)^-1] loading rate of organic N into the stream
    W_Na = (LU_Na + Na_inc*20)*Q_st # [g (6h)^-1] loading rate of ammonium N into the stream
    W_Ni = (LU_Ni + Ni_inc*20)*Q_st # [g (6h)^-1] loading rate of nitrate N into the stream

    
    # Specify nitrogen loading from the upstream land use based on the number of years that have passed. 
    # After 20 years, each scenario will reach their expected loading rate for that land use going forward.
    if(t < 20){
      W_No = (LU_No + No_inc*yr_curr)*Q_st # [g (6h)^-1] loading rate of organic N into the stream
      W_Na = (LU_Na + Na_inc*yr_curr)*Q_st # [g (6h)^-1] loading rate of ammonium N into the stream
      W_Ni = (LU_Ni + Ni_inc*yr_curr)*Q_st # [g (6h)^-1] loading rate of nitrate N into the stream
    } else {
      W_No = (LU_No + No_inc*20)*Q_st # [g (6h)^-1] loading rate of organic N into the stream
      W_Na = (LU_Na + Na_inc*20)*Q_st # [g (6h)^-1] loading rate of ammonium N into the stream
      W_Ni = (LU_Ni + Ni_inc*20)*Q_st # [g (6h)^-1] loading rate of nitrate N into the stream
    ...
    if (t > 2) {
      if(interval %% 366 < 91 | interval %% 366 > 334) { 
        ...
        catch_rate = 1.05*Fmsy_mugil # harvest after 2 years during times of year outside of the closed season
        }
    }
    else {
...
state <- c(
           ...
           Na_fp = 0.03, # [g/m^3] fishpond initial ammonium, estimated from mean water quality data Frank (2016)
           Ni_fp = 0.68, # [g/m^3] fishpond initial nitrate, estimated from mean water quality data Frank (2016)
...
out_control <- solveFish(pars = pars_control)
...
parRanges_control <- data.frame(min = c(0, 0, 0, 670/4*.8, 74/4*.8, K_min, K_hs_fish*.8, No_ocean*.8, Na_ocean*.8, Ni_ocean*.8, sp_fl*.8, sp_ebb*.8, np_fl*.8, np_ebb*.8, K_alg*.8, r_mugil*.8, M_mugil*.8, Fmsy_mugil*.8), 
                        max = c(0, 0, 0, 670/4*1.2, 74/4*1.2, K_max, K_hs_fish*1.2, No_ocean*1.2, Na_ocean*1.2, Ni_ocean*1.2, sp_fl*1.2, sp_ebb*1.2, np_fl*1.2, np_ebb*1.2, K_alg*1.2, r_mugil*1.2, M_mugil*1.2, Fmsy_mugil*1.2))
...
parRanges_restored <- data.frame(min = c(0.22*.8, 0.009*.8, 0.009*.8, 670/4*.8, 74/4*.8, K_min, K_hs_fish*.8, No_ocean*.8, Na_ocean*.8, Ni_ocean*.8, sp_fl*.8, sp_ebb*.8, np_fl*.8, np_ebb*.8, K_alg*.8, r_mugil*.8, M_mugil*.8, Fmsy_mugil*.8), 
                        max = c(0.22*1.2, 0.009*1.2, 0.009*1.2, 670/4*1.2, 74/4*1.2, K_max, K_hs_fish*1.2, No_ocean*1.2, Na_ocean*1.2, Ni_ocean*1.2, sp_fl*1.2, sp_ebb*1.2, np_fl*1.2, np_ebb*1.2, K_alg*1.2, r_mugil*1.2, M_mugil*1.2, Fmsy_mugil*1.2))
...
```{r}
parRanges_urban <- data.frame(min = c(0.56*.8, 0.023*.8, 0.023*.8, 670/4*.8, 74/4*.8, K_min, K_hs_fish*.8, No_ocean*.8, Na_ocean*.8, Ni_ocean*.8, sp_fl*.8, sp_ebb*.8, np_fl*.8, np_ebb*.8, K_alg*.8, r_mugil*.8, M_mugil*.8, Fmsy_mugil*.8), 
                        max = c(0.56*1.2, 0.023*1.2, 0.023*1.2, 670/4*1.2, 74/4*1.2, K_max, K_hs_fish*1.2, No_ocean*1.2, Na_ocean*1.2, Ni_ocean*1.2, sp_fl*1.2, sp_ebb*1.2, np_fl*1.2, np_ebb*1.2, K_alg*1.2, r_mugil*1.2, M_mugil*1.2, Fmsy_mugil*1.2))
...
Sns_urban <- sensFun(func = solveFish, parms = pars_urban, varscale = 1)
```
...
```{r}
parorder <- c("No_inc", "Na_inc", "Ni_inc", 
                         "sp_fl",
                         "sp_ebb",
                         "np_fl",
                         "np_ebb",
...
Sns_control_nit_df <- Sns_control_key %>% 
  ...
  filter(Parameter != "r_mugil" & Parameter != "No_inc" & Parameter != "Na_inc" & Parameter != "Ni_inc" & Parameter != "No_ocean" & Parameter != "Na_ocean" & Parameter != "Ni_ocean")
...
```{r}
Sns_fish_summ <- Sns_control_fish_df %>% 
  group_by(Parameter) %>% 
  summarise(mean_sns = mean(Sensitivity)) %>% 
  arrange(mean_sns)
...
Sns_control_all_df <- Sns_control_key %>% 
...
```{r}
Sns_control_plot <- ggplot(data = Sns_control_all_df, aes(x = x-1200, y = Sensitivity, col= Parameter)) +
  geom_line() +
  guides(fill=FALSE) +
  facet_wrap(~var) + 
  labs(x = "Time (Years)") +
...
# Model validation with field observations from Frank, 2016
```{r}
...
prot_per_area # difficult to see the mass from fish catch
```
...
fed_stacked <- ggplot(cal_sum, aes(fill = Scenario, y=ppl_per_cultha, x=Time)) + 
  ...
  scale_fill_discrete_qualitative(palette = "Cold") +
    guides(fill=guide_legend()) +
    labs(y = "People Fed per Hectare", x = "Years") +
    scale_x_continuous(expand = c(0, 0)) + 
    scale_y_continuous(expand = c(0, 0)) +
    theme(axis.text.x= element_text(size = 8, hjust = .2), panel.grid.major = element_blank(), panel.grid.minor = element_blank(),panel.background = element_blank(), axis.line = element_line(colour = "black"), plot.title = element_text(hjust = 0.5), legend.title.align = 0.5, strip.background = element_blank())

fed_stacked # difficult to see the mass from fish catch
```
